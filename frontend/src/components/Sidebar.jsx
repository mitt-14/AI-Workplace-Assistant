  ...
  return (
    <>
      ...
      >
          ...
          {items.map(([id, label, Icon]) => {
            ...
            return (
              ...
              >
                ...
                <Icon
                  size={18}
                  className={active ? "text-indigo-300" : "transition group-hover:text-zinc-300"}
                />
                <span>{label}</span>
                {id === "agent" && (
                  <Sparkles size={13} className="ml-auto text-fuchsia-400/70" />
                )}
              </button>
            );
          })}
        ...
        <div className="absolute bottom-5 left-4 right-4 rounded-2xl border border-white/[0.07] bg-white/[0.035] p-4">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-xs font-semibold text-zinc-300">Local AI stack</p>
            <span className="h-2 w-2 rounded-full bg-emerald-400" />
          </div>
          <p className="text-[11px] leading-5 text-zinc-600">
            FastAPI · Ollama · SQLite · ChromaDB
          </p>
        </div>
      </aside>
    </>
